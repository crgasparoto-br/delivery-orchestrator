...
function auditPrompt(request) {
  return [
    ...
    '',
    'CANDIDATE.diff is a deterministic bounded subset of the exact base-to-candidate unified diff. DIFF_MANIFEST.json binds the full diff by SHA-256 and byte count, derives each represented path from its own unified-diff header, lists every changed path, and records included or omitted diff blocks with explicit reasons. Within the fixed byte ceiling, the selector reserves one eligible representative from each semantic class before any spillover so executable source cannot starve tests, contract/config, evidence, canonical docs, or active worker prompts. MATERIAL_CONTEXT.json is supplemental: only when diff-path alignment is exact does it suppress bounded-diff representatives; otherwise it re-reads changed material instead of trusting an unverified positional association. Generated locks never displace semantic material; every changed path remains represented by the bounded diff, supplemental context, or an explicit omission reason. Respect both manifests and their limits. If a release-blocking conclusion genuinely depends on still-omitted diff or file context, report a concrete audit-context-insufficient finding instead of guessing or browsing outside the bundle.',
    '',
...
export async function main() {
  ...
  const representedPaths = diffEvidence.manifest.alignmentExact
