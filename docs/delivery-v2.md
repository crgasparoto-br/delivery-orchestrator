...
Successful refreeze persists the same PR/head, real classifier and CI references with phase `post-write-refreeze`. Independent audit, technical hygiene and unknown historical audit budgets remain blocking requirements; refreeze does not grant release readiness or infer missing producer identity. No AI or implementation worker is needed for deterministic refreeze. Write capability is resolved only for an actual adoption/terminal-state mutation; ordinary read-only guard paths need no write token. Exhausted pre-material bootstrap leases are persisted as terminal, retaining counters, scope binding and observed worker failure information.
