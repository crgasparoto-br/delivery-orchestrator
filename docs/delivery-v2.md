...
These ceilings are not increased by bootstrap recovery. After `escalated-initial-budget-exhausted`, the deterministic controller may grant exactly one recovery dispatch per verified control-plane SHA epoch only for a proven eligible `pre-material` infrastructure/unknown failure. In addition, a persisted `reserved-initial-attempt` whose correlated worker completed `success` but produced no managed PR may be re-armed after a verified control-plane SHA change. That successful-but-non-material execution is replaced in place: reservation restores the same implementation-attempt number rather than consuming the next slot. Recovery provenance preserves the prior/current controller SHAs and prior attempt count. Same-SHA re-entry never grants this recovery; the existing worker is recovered instead, and ambiguous provenance fails closed.
