...
These ceilings are not increased by bootstrap recovery. After `escalated-initial-budget-exhausted`, the deterministic controller may grant exactly one recovery dispatch per verified control-plane SHA epoch only for a proven eligible `pre-material` infrastructure/unknown failure; the same control-plane SHA remains `human-escalation`. In addition, a persisted `reserved-initial-attempt` whose correlated worker completed `success` but produced no managed PR may be re-armed after a verified control-plane SHA change. That successful-but-non-material execution is replaced in place: reservation restores the same implementation-attempt number rather than consuming the next slot. Recovery provenance preserves the prior/current controller SHAs and prior attempt count. Same-SHA re-entry for that successful-worker path never grants a new recovery; the existing worker is recovered instead, and ambiguous provenance fails closed.
...
The dispatcher checks out the adopted PR's immutable head and routes `resume_pr` to deterministic `post-write-refreeze`, excluding initial reservation and implementation. An explicit Delivery V2 dispatch may provide the exact-identity continuation request directly from the trusted controller run, bound to the live repository/issue/PR/base/head identity and controller-run evidence. Existing trusted `delivery-v2-audit-continuation` JSON comments remain supported for compatibility. Both forms request only `handoff-stale`, `post-write-refreeze`, `requires_refreeze=true`, `next_phase=finalize-after-ci`, and exact-head CI reuse; neither is independent audit approval. Classification uses the canonical classifier and observed changed paths. CI reuse requires a terminal successful run/check for the current SHA, configured repository/workflow, branch and matching GitHub Actions check suite. Head/base drift invalidates continuation and downstream evidence, including a final recheck before persistence. See [the continuation contract](delivery-v2/LEGACY_ADOPTION.md).
