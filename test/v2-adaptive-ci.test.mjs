...
test('FAST web component stays fast only through an explicit repository safe root', () => {
...
test('unknown path fails closed to critical', () => {
  const plan = buildCiPlan({
    requested: 'auto',
    changedPaths: ['tooling/new-surface.xyz'],
    repositoryPolicy: WEB_POLICY
  });
  assert.equal(plan.riskProfile, 'critical');
...
test('static extension outside a fast safe root cannot select focused CI', () => {
  const plan = buildCiPlan({
    requested: 'auto',
    changedPaths: ['infra/diagram.svg'],
    repositoryPolicy: WEB_POLICY
  });
  assert.equal(plan.riskProfile, 'critical');
  assert.equal(plan.ciMode, 'full');
});

test('explicit standard is preserved for otherwise fast files', () => {
  const plan = buildCiPlan({
    requested: 'standard',
    changedPaths: ['docs/delivery-note.md'],
    repositoryPolicy: WEB_POLICY
  });
...
test('adaptive CI reusable workflow exposes deterministic outputs and repository policy input without AI execution', async () => {
  const body = await readFile('.github/workflows/delivery-v2-classify-ci.yml', 'utf8');
  assert.match(body, /workflow_call:/);
  assert.match(body, /risk_policy_json:/);
  for (const output of [
  ...
  assert.match(body, /repository-policy-json:/);
  assert.match(body, /actions\/delivery-v2-risk@main/);
  assert.doesNotMatch(body, /gh-aw|codex|claude|copilot/i);
});

test('risk action is dependency-free, parses repository policy and delegates policy to the V2 source of truth', async () => {
  ...
  assert.match(metadata, /repository-policy-json:/);
  assert.match(source, /pulls\/\$\{pullRequestNumber\}\/files/);
  assert.match(source, /repository-policy-json/);
