...
15. Tratar auditoria independente como confirmacao de uma defesa ja exercitada internamente: nenhuma familia de risco material pode estrear somente depois do handoff se era derivavel do contrato, do diff ou de um `audit_escape` anterior.
...
17. Antes do primeiro handoff independente, todo requisito coberto deve aparecer em `requirement-attack-matrix.json` com implementacao errada plausivel, controle positivo, controle negativo discriminante, casos irmaos e regressao executados no SHA final.
18. Classificar explicitamente todas as familias canonicas em `risk-saturation.json`; familia material aplicavel sem controle impede freeze/handoff.
...
5. Para cada requisito material, formular antes da implementacao uma `plausible_wrong_implementation` que passaria no caminho feliz e derivar a familia de ataque correspondente.
6. Inicializar `requirement-attack-matrix.json`; consultar `audit-escape-pattern-catalog.json` e ativar padroes anteriores cujos sinais aparecam no contrato atual.
...
4. Executar `validate_specification_coverage.py`, `validate_requirement_attack_matrix.py`, `validate_risk_saturation.py` e `validate_inherited_controls.py` sobre o SHA final. A matriz de risco deve explicitar inclusive familias `not-applicable`.
...
9. Produzir `audit-escape-closure.json` para cada escape e exigir `status=passed`; incorporar cada `escape_class` em `audit-escape-pattern-catalog.json` e seus controles em `inherited-controls.json` antes de novo freeze/handoff.
